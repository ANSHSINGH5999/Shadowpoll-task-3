...
export const runDeploy = async (
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<"castVote">(config.zkConfigPath);
    const providers: MidnightProviders<"castVote", PrivateStateId, ShadowPollPrivateState> = {
    ...
    logger.info(`Deploying ShadowPoll contract with question: "${pollQuestion}"`);
