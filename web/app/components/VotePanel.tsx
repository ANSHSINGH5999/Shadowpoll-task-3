...
export function VotePanel({ networkId, contractAddress }: VotePanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-[var(--border)] pt-6">
      {walletState !== "connected" ? (
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="shrink-0 rounded-full bg-[var(--foreground)] px-4 py-2 text-sm font-medium text-[var(--background)] disabled:opacity-50"
          >
        ...
        <div className="flex flex-col gap-3">
          <div className="text-xs text-[var(--muted)]">
            Connected as <span className="font-mono text-[var(--foreground)]">{truncateMiddle(address ?? "")}</span>
          </div>
          <div className="flex gap-3">
            <button
              ...
              disabled={voting}
              className="flex-1 rounded-full border border-[var(--series-yes)] px-4 py-2 text-sm font-medium text-[var(--series-yes)] disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={voting}
              className="flex-1 rounded-full border border-[var(--series-no)] px-4 py-2 text-sm font-medium text-[var(--series-no)] disabled:opacity-50"
            >
              {voting ? "Submitting…" : "Vote No"}
            </button>
          </div>
          {voteState === "submitted" || voteState === "confirmed" ? (
            <p className="text-xs text-[var(--series-yes)]">
          ...
          {voteState === "already-voted" ? (
            <p className="text-xs text-[var(--series-no)]">
      ...
      {errorMessage ? <p className="text-xs text-[var(--series-no)]">{errorMessage}</p> : null}
    </div>
  );
}
